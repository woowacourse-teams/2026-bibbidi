...
public class UserMapper {
    ...
    public JpaUserEntity toEntity(User user) {
        return new JpaUserEntity(user.id(), user.nickname(), user.passwordLoginId(), user.passwordHash(), null);
    }

    public User toDomain(JpaUserEntity entity) {
        return new User(entity.id(), entity.nickname(), entity.passwordLoginId(), entity.passwordHash());
    }
}
